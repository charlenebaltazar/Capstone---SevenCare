...
export default function EditDoctor() {
  ...
  const handleUpdateDoctor = async (e: React.FormEvent) => {
    ...
    try {
      const updateData: UpdateDoctorPayload = {
        name,
        specialization,
        schedule,
      };

      await axios.patch(`${BACKEND_DOMAIN}/api/v1/doctors/${id}`, updateData, {
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        const res = await axios.get(`${BACKEND_DOMAIN}/api/v1/doctors/${id}`, {
          headers: {
        ...
        const utcDate = new Date(res.data.data.schedule);
